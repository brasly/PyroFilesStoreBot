...
async def start(bot, cmd):
	...
	if usr_cmd == "/start":
				...
				await bot.send_message(
					...
					reply_markup=InlineKeyboardMarkup(
						[
							...
							[
								InlineKeyboardButton("🔄 Refresh / Try Again", url=f"https://t.me/{BOT_USERNAME}?start=xwfsn_{file_id}")
							]
						]
					),
					parse_mode="markdown"
		...
		try:
			...
			await send_stored_file.reply_text(f"**Here is Sharable Link of this file:** https://t.me/{BOT_USERNAME}?start=xwfsn_{file_id}\n\n__To Retrive the Stored File, just open the link!__", disable_web_page_preview=True, quote=True)
...
async def main(bot, message):
	if message.chat.type == "private":
		...
		try:
			...
			share_link = f"https://t.me/{BOT_USERNAME}?start=xwfsn_{file_er_id}"
			await editable.edit(
